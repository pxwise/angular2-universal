...
var private_export_1 = require('@angular/core/private_export');
var Console = private_export_1.__core_private__.Console, MAX_INTERPOLATION_VALUES = private_export_1.__core_private__.MAX_INTERPOLATION_VALUES, SecurityContext = private_export_1.__core_private__.SecurityContext;
var ast_1 = require('@angular/compiler/src/expression_parser/ast');
var parser_1 = require('@angular/compiler/src/expression_parser/parser');
var html_parser_1 = require('@angular/compiler/src/html_parser');
var html_tags_1 = require('@angular/compiler/src/html_tags');
...
var provider_parser_1 = require('@angular/compiler/src/provider_parser');
...
var NodeTemplateParserRc13 = (function () {
    function NodeTemplateParserRc13(_exprParser, _schemaRegistry, _htmlParser, _console, transforms) {
    ...
    NodeTemplateParserRc13.prototype.parse = function (component, template, directives, pipes, templateUrl) {
    ...
    NodeTemplateParserRc13.prototype.tryParse = function (component, template, directives, pipes, templateUrl) {
        ...
        if (errors.length > 0) {
            return new template_parser_1.TemplateParseResult(result, errors);
        }
        if (common_1.isPresent(this.transforms)) {
            this.transforms.forEach(function (transform) { result = template_ast_1.templateVisitAll(transform, result); });
        }
        return new template_parser_1.TemplateParseResult(result, errors);
    };
    NodeTemplateParserRc13 = __decorate([
        core_1.Injectable(),
        __param(4, core_1.Optional()),
        __param(4, core_1.Inject(template_parser_1.TEMPLATE_TRANSFORMS)), 
        __metadata('design:paramtypes', [parser_1.Parser, element_schema_registry_1.ElementSchemaRegistry, html_parser_1.HtmlParser, Console, Array])
    ], NodeTemplateParserRc13);
    return NodeTemplateParserRc13;
}());
exports.NodeTemplateParserRc13 = NodeTemplateParserRc13;
var TemplateParseVisitor = (function () {
    ...
    TemplateParseVisitor.prototype._reportError = function (message, sourceSpan, level) {
        ...
        this.errors.push(new template_parser_1.TemplateParseError(message, sourceSpan, level));
    };
    ...
    TemplateParseVisitor.prototype._parseTemplateBindings = function (value, sourceSpan) {
        ...
        try {
            ...
            bindingsResult.templateBindings.forEach(function (binding) {
                if (common_1.isPresent(binding.expression)) {
    ...
    TemplateParseVisitor.prototype.visitElement = function (element, parent) {
        ...
        var projectionSelector = common_1.isPresent(preparsedElement.projectAs) ?
            selector_1.CssSelector.parse(preparsedElement.projectAs)[0] :
            elementCssSelector;
        var ngContentIndex = parent.findNgContentIndex(projectionSelector);
        var parsedElement;
        if (preparsedElement.type === template_preparser_1.PreparsedElementType.NG_CONTENT) {
            if (common_1.isPresent(element.children) && element.children.length > 0) {
    ...
    TemplateParseVisitor.prototype._parseInlineTemplateBinding = function (attr, targetMatchableAttrs, targetProps, targetVars) {
        ...
        if (common_1.isPresent(templateBindingsSource)) {
            ...
            for (var i = 0; i < bindings.length; i++) {
                ...
                else if (common_1.isPresent(binding.expression)) {
    ...
    TemplateParseVisitor.prototype._parseAttr = function (isTemplateElement, attr, targetMatchableAttrs, targetProps, targetEvents, targetRefs, targetVars) {
        ...
        var bindParts = common_1.regExFirstMatch(BIND_NAME_REGEXP, attrName);
        var hasBinding = false;
        if (common_1.isPresent(bindParts)) {
            hasBinding = true;
            if (common_1.isPresent(bindParts[1])) {
                this._parseProperty(bindParts[7], attrValue, attr.sourceSpan, targetMatchableAttrs, targetProps);
            }
            else if (common_1.isPresent(bindParts[2])) {
            ...
            else if (common_1.isPresent(bindParts[3])) {
                if (isTemplateElement) {
                    var identifier = bindParts[7];
                    this._parseVariable(identifier, attrValue, attr.sourceSpan, targetVars);
                }
    ...
    TemplateParseVisitor.prototype._parseDirectives = function (selectorMatcher, elementCssSelector) {
        ...
        return directives.filter(function (dir) { return common_1.isPresent(dir); });
    };
    TemplateParseVisitor.prototype._createDirectiveAsts = function (isTemplateElement, elementName, directives, props, elementOrDirectiveRefs, sourceSpan, targetReferences) {
        ...
        elementOrDirectiveRefs.forEach(function (elOrDirRef) {
            ...
            else if (common_1.isBlank(component)) {
                var refToken = null;
                if (isTemplateElement) {
    ...
    TemplateParseVisitor.prototype._createDirectiveHostPropertyAsts = function (elementName, hostProps, sourceSpan, targetPropertyAsts) {
        var _this = this;
        if (common_1.isPresent(hostProps)) {
            Object.keys(hostProps);
            common_1.stringMapForEach(hostProps, function (expression, propName) {
    ...
    TemplateParseVisitor.prototype._createDirectiveHostEventAsts = function (hostListeners, sourceSpan, targetEventAsts) {
        var _this = this;
        if (common_1.isPresent(hostListeners)) {
            common_1.stringMapForEach(hostListeners, function (expression, propName) {
                _this._parseEvent(propName, expression, sourceSpan, [], targetEventAsts);
            });
        }
    };
    TemplateParseVisitor.prototype._createDirectivePropertyAsts = function (directiveProperties, boundProps, targetBoundDirectiveProps) {
        if (common_1.isPresent(directiveProperties)) {
            var boundPropsByName = new Map();
            boundProps.forEach(function (boundProp) {
                var prevValue = boundPropsByName.get(boundProp.name);
                if (common_1.isBlank(prevValue) || prevValue.isLiteral) {
                    // give [a]="b" a higher precedence than a="b" on the same element
                    boundPropsByName.set(boundProp.name, boundProp);
                }
            });
            common_1.stringMapForEach(directiveProperties, function (elProp, dirProp) {
                var boundProp = boundPropsByName.get(elProp);
                // Bindings are optional, so this binding only needs to be set up if an expression is given.
                if (common_1.isPresent(boundProp)) {
    ...
    TemplateParseVisitor.prototype._createElementPropertyAsts = function (elementName, props, directives) {
        ...
        props.forEach(function (prop) {
            if (!prop.isLiteral && common_1.isBlank(boundDirectivePropsIndex.get(prop.name))) {
    ...
    TemplateParseVisitor.prototype._assertAllEventsPublishedByDirectives = function (directives, events) {
        ...
        directives.forEach(function (directive) {
            common_1.stringMapForEach(directive.directive.outputs, function (eventName, _) { allDirectiveEvents.add(eventName); });
        });
        events.forEach(function (event) {
            if (common_1.isPresent(event.target) || !allDirectiveEvents.has(event.name)) {
...
var ElementContext = (function () {
    ...
    ElementContext.create = function (isTemplateElement, directives, providerContext) {
        ...
        if (common_1.isPresent(component)) {
    ...
    ElementContext.prototype.findNgContentIndex = function (selector) {
        ...
        if (common_1.isPresent(this._wildcardNgContentIndex)) {
...
//# sourceMappingURL=node_template_parser-rc1-3.js.map